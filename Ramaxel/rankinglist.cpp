...
void RankingList::initTableWidget()
{
    ...
    ui->tableWidget->horizontalHeader()->setDefaultSectionSize(300);    //列宽
    ...
    ui->tableWidget->setSelectionMode(QAbstractItemView::SingleSelection);  //当行选择
...
QByteArray RankingList::setFilesListJson(int start, int count)
{
    ...
    QJsonDocument jsonDoc = QJsonDocument::fromVariant(tmp);
    if(jsonDoc.isNull())
    {
        return "";
    }
